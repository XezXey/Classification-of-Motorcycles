...
Mat localize_operation_area(Mat operated_area_frame);
...
int video_processing(String input_filename)
{
	...
	for (;;)
	{
		...
		//rectangle(motorcycle_frame, Rect(0, 213, 640, 190), Scalar(0, 0, 255), 2, 8, 0);
...
#ifdef ANNS
		model_anns->predict(data, result_responses_non_onehot);

		for (int j = 0; j < 3; j++) {
			//convert to onehot
			result_responses_onehot.at<float>(i, j) = abs(1 - result_responses_non_onehot.at<float>(i, j)) <= 0.18 ? 1.f : 0.f;
		}
			
		if (result_responses_onehot.at<float>(i, 0) == 1.0 && result_responses_onehot.at<float>(i, 1) == 0.0 &&
			result_responses_onehot.at<float>(i, 2) == 0.0 ) {
			motorcyclename = "Fino";
		}
		else if (result_responses_onehot.at<float>(i, 0) == 0.0 && result_responses_onehot.at<float>(i, 1) == 1.0 &&
			result_responses_onehot.at<float>(i, 2) == 0.0) {
			motorcyclename = "Scoopy-i";
		}
		else if (result_responses_onehot.at<float>(i, 0) == 0.0 && result_responses_onehot.at<float>(i, 1) == 0.0 &&
			result_responses_onehot.at<float>(i, 2) == 1.0) {
			motorcyclename = "Wave";
		}
		else {
...
#endif 
		cout << "This is : " << motorcyclename << endl;
