...
//This program take 2 agrument 
//1. image as an input and calculate the HoG for each block then visualize it. And use for training ANNs
//2. filename to export hog value
#include "stdafx.h"
#include "finding_hog_features.h"
...
String window_name_hog = "HoG_Visualization";
...
void export_hog_value_to_file(vector<float> hog_value_motorcycle_roi, String input_filename_to_write)
{
	fstream	hog_value_file;
	input_filename_to_write += "hog_value_for_train.txt";
	hog_value_file.open(input_filename_to_write, ios::app);
	if (hog_value_file.is_open()) {
		cout << "Open file successfully" << endl;
	}
	else cout << "Unable to open file" << endl;
	
	for (int i = 0; i < hog_value_motorcycle_roi.size(); i++) {
		hog_value_file << hog_value_motorcycle_roi[i] << ",";
	}
	hog_value_file << endl;
	cout << "Writing Done!!!" << endl;
	hog_value_file.close();
}
